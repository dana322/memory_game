...
class FlipCardByMemoryGame():
    ...
    def __init__(self):
        ...
        # 数据库
        self.data_bases = pymysql.connect(host='8.142.100.211',
                                            port=3306, user='root',
                                            passwd='123',
                                            database='ranking')
        self.db_cursor = self.data_bases.cursor()
        # 难度标识 0 初级 1 中级 2 高级
        self.which_game = 0
        # 游戏窗口
        self.root = NULL
        ...
        self.game_matrix = {}
        # 背景图像
        self.blank_image = NULL
        # 卡片背面
        self.cards_back_image = NULL
        # 已经显示正面的卡片
        self.shown_cards = []
        # 场上存在的卡片数量
        self.num_existing_cards = 0
        # 音乐
        self.score_sound = NULL
        # 是否是第一次点击flag
    ...
    def create_login_window(self):
        ...
        self.login_window.geometry(f'+{int(x)}+{int(y)}')
        self.login_window.deiconify()

    def create_game_window(self, image_num = junior_constants.JUNIOR_IMAGE_NUM,
                            line = junior_constants.JUNIOR_LINE,
                            column = junior_constants.JUNIOR_COLUMN,
                            rank = junior_constants.JUNIOR_RANK):
        '''游戏界面'''
        ...
        # 在界面上显示所有卡片的背面
        for row_index in range(line):
            for column_index in range(column):
                position = f'{row_index}_{column_index}'
                ...
                # row_index*colmn+c生成0-cards_num个数
                self.game_matrix[position].file = str(cards_list[row_index * column + column_index])
                self.game_matrix[position].show = False
                self.game_matrix[position].bind('<Button-1>', self.clickcallback)
                self.game_matrix[position].grid(row=row_index, column=column_index)
        ...
        greet = Label(self.root, text=f'欢迎你! {self.user_name}')
        greet.grid(row=line + 1, column=0)
    ...
    def run_mediate(self):
        '''运行中级游戏'''
        self.which_game = 1
        self.create_game_window(mediate_constants.MEDIATE_IMAGE_NUM,
                                mediate_constants.MEDIATE_LINE,
                                mediate_constants.MEDIATE_COLUMN,
                                mediate_constants.MEDIATE_RANK)

    def run_advanced(self):
        '''运行高级游戏'''
        self.which_game = 2
        self.create_game_window(advanced_constants.ADVANCED_IMAGE_NUM,
                                advanced_constants.ADVANCED_LINE,
                                advanced_constants.ADVANCED_COLUMN,
                                advanced_constants.ADVANCED_RANK)
    ...
    def clickcallback(self, event):
        ...
        # 之前只有一张卡片被翻开
            ...
            if self.shown_cards[0].file == card.file:
                ...
                image = ImageTk.PhotoImage(Image.open(
                        os.path.join(self.card_dir, card.file+'.png')))
                card.configure(image=image)
                card.show_image = image
                card.show = True
        ...
        if self.num_existing_cards == 0:
            ...
            is_restart = messagebox.askyesno(
                        'Game Over', 'Congratulations, you win, do you want to play again?')
            if is_restart:
                self.restart()
            else:
                self.root.destroy()
    def insert_grade(self):
        ...
        try:
            if self.which_game == 0:
                self.db_cursor.execute(f"insert into junior_ranking \
                                        values(null, '{time}', '{self.user_name}');")
            elif self.which_game == 1:
                self.db_cursor.execute(f"insert into mediate_ranking \
                                        values(null, '{time}', '{self.user_name}');")
            elif self.which_game == 2:
                self.db_cursor.execute(f"insert into advanced_ranking \
                                        values(null, '{time}', '{self.user_name}');")
            self.data_bases.commit()
        except:
            self.data_bases.rollback()
    def tick(self):
        '''计时'''
        if self.num_existing_cards == 0: 
            return
