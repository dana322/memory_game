...
class FlipCardByMemoryGame():
    ...
    def __init__(self):
        ...
        # 新建文本标签
        self.username = Label(self.root, text = "用户名：")
        self.username.pack()
        # 创建动字符串
        Dy_String = StringVar()
        self.entry = Entry(self.root,textvariable =Dy_String,validate ="focusout",validatecommand= self.check)
        self.entry.pack()
        # 创建游客登录按钮
        # TODO:grid布局
        self.guest_login_button = Button(self.root, text="游客登录", command=self.create_game_window)
    ...
    def create_game_window(self):
        ...
        self.game_window = Tk()
        self.game_window.geometry('400x500')
        label = Label(self.game_window, text="新")
        label.pack()
        self.geme_window.mainloop()
